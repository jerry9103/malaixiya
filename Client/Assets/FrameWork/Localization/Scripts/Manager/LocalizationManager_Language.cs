...
namespace GameGoing.SDK
{
    public static partial class LocalizationManager
    {
        ...
        public static int GetLanguageIndex(string language) {
            List<string> list = GetAllLanguages();
        ...
        public static string GetLanguageDataFromTerm(string team) {
            if (Sources.Count == 0)
                UpdateSources();
            ...
            for (int i = 0, imax = Sources.Count; i < imax; ++i)
            {
                string data = Sources[i].GetTermData(team, index);
                if (!string.IsNullOrEmpty(data)) return data;
                data = Sources[i].GetTermData(team.ToLower(), index);
                if (!string.IsNullOrEmpty(data)) return data;
            }
        ...
        public static string LocalizationFromTerm(this string term) {
            string data = GetLanguageDataFromTerm(term);

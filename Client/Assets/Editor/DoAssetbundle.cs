...
public class AssetbundleEditor : MonoBehaviour
{
    ...
    static void CreateAllAssetBundles()
    {
        ...
        //打包资源
#if UNITY_IPHONE
        BuildPipeline.BuildAssetBundles(targetPath, BuildAssetBundleOptions.None, BuildTarget.iOS);     //这里设置为ios
#else
        BuildPipeline.BuildAssetBundles(targetPath, BuildAssetBundleOptions.None, BuildTarget.Android);     //这里设置为安卓
#endif
        //刷新编辑器
    ...
    /// <summary>
    /// 官方的打包案例，在此处没有正确赋值，无法使用（官方案例，在此无效）
    /// </summary>
    [MenuItem("AssetBundle/Build AssetBundles Using Building Map")]
    static void BuildMapABs()
    {
        ...
        BuildPipeline.BuildAssetBundles("Assets/ABs", buildMap, BuildAssetBundleOptions.None, BuildTarget.Android);   //打包资源到Assets下的ABs文件夹
    }
#endif
}
